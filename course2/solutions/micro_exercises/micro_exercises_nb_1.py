...
95% of the time is spent in `compute_sequence_similarity`
...
for n in Ns:
    ...
    memory_usage.append(sys.getsizeof(c)/(1024**2))  # Get the memory size of that matrix.
...
# compute the max (uni-directional) dimension N of a matrix that we can store
# in 32 GB of RAM.
np.sqrt((1024 * 32) /  mem_per_element)  # -> ~65'000 rows/columns.
# The result we obtain is that the max size N (number of rows/columns) of a
# matrix that fits in 32Gb of RAM is of about 65'000 x 65'000.
...
Don't store the whole matrix in memory.

basically the whole step 4 can be removed.
